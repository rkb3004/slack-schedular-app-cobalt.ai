...
const debugController = new debug_controller_1.DebugController(slackAuthService);
...
router.get('/slack/url', (req, res) => {
    try {
        ...
        if (!(redirectUri === null || redirectUri === void 0 ? void 0 : redirectUri.includes('/api/slack/callback'))) {
        ...
        console.log('Client ID first 5 chars:', clientId === null || clientId === void 0 ? void 0 : clientId.substring(0, 5));
        console.log('Redirect URI:', redirectUri);
        console.log('Client ID raw value:', clientId);
        console.log('Client ID trim check:', (clientId === null || clientId === void 0 ? void 0 : clientId.trim()) === clientId);
        console.log('Client ID length check:', clientId === null || clientId === void 0 ? void 0 : clientId.length);
        console.log('Client ID characters:', Array.from(clientId || '').map(c => c.charCodeAt(0)));
        const cleanClientId = clientId === null || clientId === void 0 ? void 0 : clientId.trim();
...
router.get('/slack/test-token-exchange', auth_controller_1.debugTokenExchange);
router.get('/slack/scopes', auth_controller_1.getSlackScopesInfo);
router.get('/slack/test-auth', (req, res) => {
    try {
        ...
        console.log('Test page - Client ID length:', clientId === null || clientId === void 0 ? void 0 : clientId.length);
        const authUrl = `https://slack.com/oauth/v2/authorize` +
            `?client_id=${encodeURIComponent(cleanClientId || '')}` +
            `&scope=${encodeURIComponent(scope)}` +
            `&redirect_uri=${encodeURIComponent(redirectUri || '')}`;
